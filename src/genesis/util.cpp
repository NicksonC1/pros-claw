...
float genesis::slew(float target, float current, float maxChange) {
...
constexpr float genesis::sanitizeAngle(float angle, bool radians) {
    if (radians) return std::fmod(std::fmod(angle, 2 * M_PI) + 2 * M_PI, 2 * M_PI);
    else return std::fmod(std::fmod(angle, 360) + 360, 360);
}

float genesis::angleError(float target, float position, bool radians, AngularDirection direction) {
...
float genesis::avg(std::vector<float> values) {
    float sum = 0;
    for (float value : values) { sum += value; }
    return sum / values.size();
}

float genesis::ema(float current, float previous, float smooth) {
    return (current * smooth) + (previous * (1 - smooth));
}

float genesis::getCurvature(Pose pose, Pose other) {
    // calculate whether the pose is on the left or right side of the circle
    float side = genesis::sgn(std::sin(pose.theta) * (other.x - pose.x) - std::cos(pose.theta) * (other.y - pose.y));
