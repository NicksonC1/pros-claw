...
// global variables
genesis::OdomSensors odomSensors(nullptr, nullptr, nullptr, nullptr, nullptr); // the sensors to be used for odometry
genesis::Drivetrain drive(nullptr, nullptr, 0, 0, 0, 0); // the drivetrain to be used for odometry
genesis::Pose odomPose(0, 0, 0); // the pose of the robot
genesis::Pose odomSpeed(0, 0, 0); // the speed of the robot
genesis::Pose odomLocalSpeed(0, 0, 0); // the local speed of the robot
...
void genesis::setSensors(genesis::OdomSensors sensors, genesis::Drivetrain drivetrain) {
    odomSensors = sensors;
    drive = drivetrain;
}

genesis::Pose genesis::getPose(bool radians) {
    if (radians) return odomPose;
    else return genesis::Pose(odomPose.x, odomPose.y, radToDeg(odomPose.theta));
}

void genesis::setPose(genesis::Pose pose, bool radians) {
    if (radians) odomPose = pose;
    else odomPose = genesis::Pose(pose.x, pose.y, degToRad(pose.theta));
}

genesis::Pose genesis::getSpeed(bool radians) {
    if (radians) return odomSpeed;
    else return genesis::Pose(odomSpeed.x, odomSpeed.y, radToDeg(odomSpeed.theta));
}

genesis::Pose genesis::getLocalSpeed(bool radians) {
    if (radians) return odomLocalSpeed;
    else return genesis::Pose(odomLocalSpeed.x, odomLocalSpeed.y, radToDeg(odomLocalSpeed.theta));
}

genesis::Pose genesis::estimatePose(float time, bool radians) {
...
void genesis::update() {
    ...
    genesis::TrackingWheel* verticalWheel = nullptr;
    genesis::TrackingWheel* horizontalWheel = nullptr;
    ...
    // save previous pose
    genesis::Pose prevPose = odomPose;
