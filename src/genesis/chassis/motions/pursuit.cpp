 ...
 * @return std::vector<genesis::Pose> vector of points on the path
 */
std::vector<genesis::Pose> getData(const asset& path) {
    std::vector<genesis::Pose> robotPath;
    ...
    for (std::string line : dataLines) {
        genesis::infoSink()->debug("read raw line {}", stringToHex(line));
        ...
        if (pointInput.size() != 3) {
            genesis::infoSink()->error("Failed to read path file! Are you using the right format? Raw line: {}",
                                      stringToHex(line));
            break;
        }
        genesis::Pose pathPoint(0, 0);
        ...
        genesis::infoSink()->debug("read point {}", pathPoint);
    }
 ...
 */
int findClosest(genesis::Pose pose, std::vector<genesis::Pose> path) {
    int closestPoint;
 ...
 */
float circleIntersect(genesis::Pose p1, genesis::Pose p2, genesis::Pose pose, float lookaheadDist) {
    // calculations
    // uses the quadratic formula to calculate intersection points
    genesis::Pose d = p2 - p1;
    genesis::Pose f = p1 - pose;
    float a = d * d;
    float b = 2 * (f * d);
 ...
 */
genesis::Pose lookaheadPoint(genesis::Pose lastLookahead, genesis::Pose pose, std::vector<genesis::Pose> path, int closest,
                            float lookaheadDist) {
    ...
    for (int i = start; i < path.size() - 1; i++) {
        genesis::Pose lastPathPose = path.at(i);
        genesis::Pose currentPathPose = path.at(i + 1);

        float t = circleIntersect(lastPathPose, currentPathPose, pose, lookaheadDist);

        if (t != -1) {
            genesis::Pose lookahead = lastPathPose.lerp(currentPathPose, t);
            lookahead.theta = i;
            return lookahead;
        }
    }
 ...
 */
float findLookaheadCurvature(genesis::Pose pose, float heading, genesis::Pose lookahead) {
    // calculate whether the robot is on the left or right side of the circle
    float side = genesis::sgn(std::sin(heading) * (lookahead.x - pose.x) - std::cos(heading) * (lookahead.y - pose.y));
...
void genesis::Chassis::follow(const asset& path, float lookahead, int timeout, bool forwards, bool async) {
    ...
    std::vector<genesis::Pose> pathPoints = getData(path); // get list of path points
