...
Using the Path Generator is simple:
 - Select the genesis format (top right).
...
Almost there! Now we just need to tell the robot to follow the path. We can do this through the `genesis::Chassis::follow` function. Below is an example of how to use it:
```cpp
